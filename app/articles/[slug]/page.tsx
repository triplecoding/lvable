...
export default async function ArticleDetailPage({ params }: Props) {
  const article = await api.getArticleBySlug(params.slug);
  const related = (await api.getArticles()).filter((a) => a.slug !== article.slug).slice(0, 3);
  const toc = article.content.split('\n').filter((line) => line.startsWith('##')).map((l) => l.replace('##', '').trim());

  return (
    <article className="space-y-8">
      <SchemaScript schema={articleSchema({ title: article.title, description: article.excerpt, path: `/articles/${article.slug}`, datePublished: article.publishedAt })} />
      <SchemaScript schema={faqSchema(article.faq)} />
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5">
        ...
        <div className="mt-3 grid gap-2">
          {related.map((item) => (
            <Link key={item.id} href={`/articles/${item.slug}`} className="text-brand-700 hover:underline">
              {item.title}
            </Link>
          ))}
        </div>
